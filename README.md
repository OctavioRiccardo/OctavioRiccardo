<!-- SELECCIÓN DE IDIOMA / LANGUAGE SELECTION -->
<p align="right">
  <a href="#-español">🇪🇸 Español</a> | 
  <a href="#-english">🇬🇧 English</a>
</p>

---

## 🇪🇸 Español

# Octavio Riccardo
### Estudiante de Tecnología Web & Desarrollador en Formación

Formatura orientada al desarrollo de software eficiente, escalable y con buenas prácticas. Actualmente cursando la **Tecnicatura Universitaria Web** en la **Universidad Nacional de San Luis (UNSL)**, profundizando en la ingeniería de software tanto en entornos Frontend como Backend.

- **Educación:** Tecnicatura Universitaria Web — Universidad Nacional de San Luis.
- **Enfoque Actual:** Especializándome en el ecosistema de **React** y **TypeScript** para el frontend.
- **Especialización Backend:** Desarrollando soluciones robustas mediante **Node.js** y **Spring Boot (Java)**.

<details>
  <summary>Objetivos Profesionales y Áreas de Interés</summary>
  <br>
  
  - **Desarrollo Backend:** Interesado en la arquitectura de microservicios, el diseño de APIs RESTful y la gestión eficiente de bases de datos.
  - **Desarrollo Frontend:** Enfoque en la creación de interfaces de usuario optimizadas, modulares y con tipado fuerte utilizando TypeScript.
  - **Metodologías:** Comprometido con el aprendizaje de patrones de diseño, código limpio (Clean Code) y control de versiones avanzado.
</details>

### Contacto Profesional
<p align="left">
  <a href="https://www.linkedin.com/in/octavio-riccardo/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin&logoColor=white" alt="LinkedIn">
  </a>
  <a href="mailto:octariccardo@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white" alt="Email">
  </a>
</p>

---

## 🇬🇧 English

# Octavio Riccardo
### Web Technology Student & Aspiring Software Developer

Results-oriented software development profile focused on efficiency, scalability, and best practices. Currently pursuing a **Web University Degree** at **Universidad Nacional de San Luis (UNSL)**, deepening core software engineering principles across both Frontend and Backend ecosystems.

- **Education:** Web University Degree — Universidad Nacional de San Luis.
- **Current Focus:** Specializing in the **React** and **TypeScript** ecosystem for frontend development.
- **Backend Specialization:** Building robust solutions utilizing **Node.js** and **Spring Boot (Java)**.

<details>
  <summary> Career Goals & Areas of Interest</summary>
  <br>
  
  - **Backend Development:** Interested in microservices architecture, RESTful API design, and efficient database management.
  - **Frontend Development:** Focus on building optimized, modular, and strongly-typed user interfaces using TypeScript.
  - **Methodologies:** Committed to mastering software design patterns, Clean Code principles, and advanced version control.
</details>

###  Professional Network
<p align="left">
  <a href="https://www.linkedin.com/in/octavio-riccardo/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin&logoColor=white" alt="LinkedIn">
  </a>
  <a href="mailto:octariccardo@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white" alt="Email">
  </a>
</p>

---

<!-- SECCIÓN GLOBAL: TECNOLOGÍAS Y ESTADÍSTICAS -->

##  Stack Tecnológico / Tech Stack

<table>
  <tr>
    <td align="center" width="33%"><strong>Frontend</strong></td>
    <td align="center" width="33%"><strong>Backend</strong></td>
    <td align="center" width="33%"><strong>Lenguajes / Otros</strong></td>
  </tr>
  <tr>
    <td valign="top" align="center">
      <img src="https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB" alt="React"/><br>
      <img src="https://img.shields.io/badge/JavaScript-323330?style=flat-square&logo=javascript&logoColor=F7DF1E" alt="JavaScript"/>
    </td>
    <td valign="top" align="center">
      <img src="https://img.shields.io/badge/Spring%20Boot-6DB33F?style=flat-square&logo=springboot&logoColor=white" alt="Spring Boot"/><br>
      <img src="https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white" alt="Node.js"/>
    </td>
    <td valign="top" align="center">
      <img src="https://img.shields.io/badge/TypeScript-007ACC?style=flat-square&logo=typescript&logoColor=white" alt="TypeScript"/><br>
      <img src="https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white" alt="Java"/><br>
      <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" alt="Python"/><br>
      <img src="https://img.shields.io/badge/C-00599C?style=flat-square&logo=c&logoColor=white" alt="C"/>
    </td>
  </tr>
</table>

### Desempeño y Métricas / GitHub Analytics
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=TU_USUARIO_AQUÍ&show_icons=true&theme=dark&hide_border=true&count_private=true" alt="GitHub Stats" />
  <br><br>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=TU_USUARIO_AQUÍ&layout=compact&theme=dark&hide_border=true" alt="Top Languages" />
</p>
